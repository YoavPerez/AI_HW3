...
class SplittingTest:
    ...
    def __call__(self,value):
        """
        Can be called as a function, returns the boolean value of the test.csv
        """
...
class ExampleSet:
    ...
    def initialize_from_file(self,filename):
        """
        initialize this example set from a file.txt as specified by the exercise
        """
        ...
        # parse the rest of the file.txt containing the examples
        self.examples = []
        self.positives = 0
        self.negatives = 0
    ...
    def best_split_numerical(self,attribute):
        """
        compute the best splitting test.csv for the numerical attribute ``attribute``
    ...
    def best_split_categorical(self,attribute):
        """
        compute the best splitting test.csv for the categorical attribute ``attribute``
        this is done by the algorithm given on the exercise sheet
        """
        max_information_gain = None
        splitting_test = None
    ...
    def split_boolean(self,attribute):
        ...
        #print("Information gain: {}, splitting test.csv: {}".format(information_gain,splitting_test))
        return [information_gain,splitting_test]


    def get_split(self,attribute):
        """
        Computes the best splitting test.csv for the given attribute. This method is a wrapper for the
        ...
        else: # there is only boolean values left which have only one possible splitting test.csv
    ...
    def split(self,test):
        """
        Split the example_set in examples which fullfill the test.csv and those which do not fullfill the test.csv.
        Two example sets are returned. The first one containes the examples which fullfill the test.csv.
        """
...
def TDIDT(node_list, attribute_list, current_node_idx):
    ...
    # split the example set with the given test.csv with the maximum information gain
...
if __name__ == '__main__':
    # create Example Set from given file.txt
    e = ExampleSet()
    e.initialize_from_file(pandas.read_csv('train.csv'))
    ...
    # classify the test.csv examples and cound how many were corretly classified
